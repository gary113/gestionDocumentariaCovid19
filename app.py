...
@app.route("/tabla")
def contenidotabla():
    # connection = pypyodbc.connect('Driver={MySQL ODBC 3.51 Driver};Server=.;Database=BD_PAGINA')  # Creating Cursor
...
def uploader():
    if request.method == "POST":
        ...
        # connection1 = pypyodbc.connect('Driver={MySQL ODBC 3.51 Driver};Server=.;Database=BD_PAGINA')  # Creating Cursor
...
def login():
    if request.method == 'POST':
        ...
        elif password.decode("utf-8") == str(user['PASSWORD']):
